...
class GruSettings:
    ...
    feature_count = 3  # Uses the first `feature_count` selected columns as features, ignored if `lookback_as_features`

    nTrain = None #chosen automatically if not specified

    stateful=True

    ignore_for_error = [5000,10000]
...
def build_model(input_dim, input_steps, kernel_size, levels, units, lr, dropout, loss, train_data):
    channel_sizes = [units] * levels
    (ins, outs) = build_tcn(input_dim, channel_sizes, kernel_size=kernel_size, dropout=dropout,
    ...
    data_based_init(model, train_data[:100])
    return model
...
def run_gru(s):
    ...
    model = build_model(3, s.lookback, 7,8,30, s.lr, 0, 'mae', trainX)
    ...
    if s.implementation == "keras":
        ...
        for _ in range(1):
            ...
            model.fit(trainX, trainY, epochs=s.epochs,
    ...
    buffer = 0
    for i in tqdm(range(s.nTrain + s.predictionStep, len(allX)), disable=s.max_verbosity == 0):
        if i % s.retrain_interval == 0 and s.online and i > s.nTrain + s.predictionStep + buffer:
            ...
            if s.implementation == "keras":
                ...
                for _ in range(1):
                    ...
                    model.fit(trainX, trainY, epochs=s.epochs_retrain,
                               batch_size=s.batch_size)
                    #if s.stateful:
                    #    rnn_layer.reset_states()
        ...
        if s.lookback:
            if s.implementation == "keras":
                predictedInput[i] = model.predict(np.reshape(allX[i], s.predict_input_shape))
            ...
            new_predicts = model.predict(np.reshape(allX[start:up_to], (1, -1, s.x_dims)))
